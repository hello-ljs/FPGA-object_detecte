...
//    softmax4(conf, size/4, conf_scale, conf_softmax);
	...
	sprintf(image,"/root/detected_object/people/%s%d%s%","people",++i,".jpg");
        imwrite(image,img);
        // imshow("plate",people);
        //waitKey(1000);
	...
	sprintf(image_1,"/root/detected_object/vehicle/%s%d%s%","vehicle",++j,".jpg");
        imwrite(image_1,img);
      ...
      cout << x_min << "\t" << y_min << "\t" << x_max << "\t" << y_max << "\t"
        ...
        sprintf(image_2,"/root/detected_object/mechanical/%s%d%s%","mechanical",++k,".jpg");
        imwrite(image_2,img);
