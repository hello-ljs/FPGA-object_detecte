...
const float VEHICLE_THRSHOLD = 0.6;
...
//    softmax4(conf, size/4, conf_scale, conf_softmax);
    ...
    for (size_t i = 0; i < results.size(); ++i) {
      ...
      //     << results[i][5] << endl;
      if (label_index == 1 && results[i][5] > PEOPLE_THRSHOLD) {  // people
      ...
      cout << x_min << "\t" << y_min << "\t" << x_max << "\t" << y_max << "\t"
        ...
        //Mat people;//add to save the detected images
         // Rect rect1(x_min,y_min,x_max-x_min,y_max-y_min);
         // Rect rect1(0,299,,299);//where to crop the images
         //img(rect1).copyTo(people);
	//sprintf(image,"/root/detected_images/%s%d%s%","people",++i,".jpg");
        //imwrite(image,img);
        // imshow("plate",people);
        //waitKey(1000);
        
      } else if (label_index == 2 && results[i][5] > VEHICLE_THRSHOLD) {  // vehicle 
        rectangle(img, cvPoint(x_min, y_min), cvPoint(x_max, y_max),
      ...
      cout << x_min << "\t" << y_min << "\t" << x_max << "\t" << y_max << "\t"
        ...
        //Mat vehicle;//add to save the detected images
         // Rect rect1(x_min,y_min,x_max-x_min,y_max-y_min);
         // Rect rect1(0,299,0,299);
          //img(rect1).copyTo(vehicle);
	//sprintf(image_1,"/root/detected_images/%s%d%s%","vehicle",++j,".jpg");
        //imwrite(image_1,img);
       // imshow("plate",vehicle);
        //waitKey(1000);

      } else if (label_index == 3 && results[i][5] > MECHANICAL_THRSHOLD) {  //mechanical 
        rectangle(img, cvPoint(x_min, y_min), cvPoint(x_max, y_max),
        ...
        //Mat mechanical;//add to save the detected images
         // Rect rect1(x_min,y_min,x_max-x_min,y_max-y_min);
         // Rect rect1(0,299,0,299);
          //img(rect1).copyTo(mechanical);
        //sprintf(image_2,"/root/detected_images/%s%d%s%","mechanical",++k,".jpg");
        //imwrite(image_2,img);
       // imshow("plate",mechanical);
        //waitKey(1000);
      } else {  // background
      }
	 ...
	 //sprintf(image,"/root/detected_object/%s%d%s%","detected_obj",++k,".jpg");
      ...
      // if (results[i][5] > VEHICLE_THRSHOLD)
        // sprintf(image_1,"/root/detected_object/vehicle/%s%d%s%","vehicle",++m,".jpg");
         //imwrite(image_1,img); 
       //if (results[i][5] > MECHANICAL_THRSHOLD)
         //sprintf(image_2,"/root/detected_object/mechanical/%s%d%s%","mechanical",++n,".jpg");
         //imwrite(image_2,img); 
